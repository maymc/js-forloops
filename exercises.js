...
console.log("originArray: ", valuesArray);
console.log("destinationArray: ", copyValuesArray);
...
function longestWord(phrase){
	...
	for(var k=0; k<quoteArray.length; k++){
		if(quoteArray[k].length > longWord.length)
			longWord = quoteArray[k];
	}	
...
console.log(guide);
console.log("guide length: ", guide.length);

function cityLandmark(arr){

	for(var i=0; i<guide.length; i++){
		for(var k=0; k<guide[i].length; k++){
			console.log(guide[i][k]);
		}
	}
}
